# Graph Report - peluqueria-portal  (2026-09-19)

## Corpus Check
- 89 files · ~562,422 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 8 file(s) not represented in the graph (top: .css 5, (none) 2, .example 1)

## Summary
- 746 nodes · 1352 edges · 68 communities (13 shown, 55 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 146 edges (avg confidence: 0.85)
...
- **Why does `Exception` connect `Exception` to `.smtpConnect`, `MailerService`, `SMTP`, `Servicio`, `InscripcionRepository`, `.preSend`, `.count`, `.createBody`, `Database`, `.addOrEnqueueAnAddress`, `ProfesionalService`?**
  _High betweenness centrality (0.182) - this node is a cross-community bridge._
- **Why does `PHPMailer` connect `PHPMailer` to `.smtpConnect`, `MailerService`, `Exception`, `.preSend`, `.count`, `.createBody`, `.addOrEnqueueAnAddress`?**
  _High betweenness centrality (0.136) - this node is a cross-community bridge._
- **Why does `FPDF` connect `FPDF` to `._parsepngstream`, `._textstring`, `MailerService`, `Exception`, `._put`, `._out`, `.count`, `.AddPage`, `.AddFont`?**
